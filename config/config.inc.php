<?php
/**
 * dynamic-dynamic-quiz by filipe knoedt
 * https://github.com/fknoedt/dynamic-dynamic-quiz
 *
...
if($_SERVER['REMOTE_ADDR'] == PRODUCTION_SERVER_IP) {
	$environment = 'prod';
	$debugMode = false;
	$errorReporting = 0;
}
...
// session will handle current responses [and..?]
session_start();
...
// QuizApi singleton
\DynamicQuiz\Quiz::$quizApi = new \DynamicQuiz\OpenTdbQuizApi();

// unsets the config.ini array for security purposes
unset($aConfig);

// return to the index with all set
